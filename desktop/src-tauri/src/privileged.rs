//! Privilege handling for write actions.
//!
//! On Linux, root-only operations go through the existing `dmgr-polkit-helper`
//! via `pkexec`, falling back to a direct `dmgr-core` call when already root.
//!
//! Windows has no polkit/pkexec: privileged device actions (Enable/Disable)
//! self-elevate through UAC inside the Windows backend, so here we only report
//! whether the current process is already running elevated (Administrator).
