...
fn handle_bind(dev_id: String, path: String, driver: String, tx: &mpsc::Sender<BgResult>) {
    let result = do_privileged(|| control::bind_driver(&path, &driver), &["bind", &path, &driver]);

    if result.is_ok() {
        // Refresh device list after successful bind
...
fn handle_unbind(dev_id: String, path: String, tx: &mpsc::Sender<BgResult>) {
    let result = do_privileged(|| control::unbind_driver(&path), &["unbind", &path]);
...
fn handle_set_property(
    ...
    let result = do_privileged(
        || properties::set_property(&path, &attr, &value),
        &["set", &path, &attr, &value],
    );
...
/// Try the operation directly; if permission denied, escalate via pkexec.
fn do_privileged<F>(direct: F, pkexec_args: &[&str]) -> Result<(), String>
where
    F: FnOnce() -> dmgr_core::error::Result<()>,
{
    match direct() {
        Ok(()) => Ok(()),
        Err(DmgrError::PermissionDenied(_)) => run_pkexec(pkexec_args),
    ...
    match status {
        ...
        Ok(s) => Err(format!(
            "pkexec dmgr-polkit-helper failed (exit {}). \
             Make sure dmgr-polkit-helper is installed in /usr/bin.",
            s.code().unwrap_or(-1)
        )),
        Err(e) => Err(format!(
            "Could not run pkexec: {}. \
             Install dmgr-polkit-helper or run dmgr as root.",
            e
        )),
    }
}
